#!/usr/bin/env python3
"""Validate and apply the exact-identity Cao Barents shelf lifecycle repair."""
...
SCOPE_CLAUSE = (
    "Three exact-identity Cao Barents/Svalbard continental-outline shelf lifecycles use "
...
def load_and_validate(package: Path):
    ...
    if any(manifest["frame"].get(key) != value for key, value in frame.items()):
        raise BuildError("package and Barents shelf frame differ")
    ...
    for row in contract["charts"]:
        ...
        if compiled_valid[0] != 410 or authority_valid[0] != 600:
            raise BuildError(f"{row['sourceFeatureId']}: expected 410/600 Ma lifecycle mismatch")
...
def build_motion_rows(contract: dict):
    ...
    youngest, oldest = bounds["youngest"], bounds["oldest"]
    ...
    output = {}
    for plate in sorted({row["plateId"] for row in contract["charts"]}):
    ...
    return cp, rotation, source_clock, output
...
def validate_applied(package: Path) -> dict:
    ...
    oracle_age = None
    lifecycle = {}
    for age in (0, 409, 422, 600, 600.000001):
        lifecycle[str(age)] = [active(chart_by_id[row["chartId"]]["lifecycle"], age)
                               for row in contract["charts"]]
    expected_lifecycle = {
        "0": [True] * 3, "409": [True] * 3, "422": [True] * 3,
        "600": [True] * 3, "600.000001": [False] * 3,
    }
    ...
    for row in contract["charts"]:
        ...
        expected_bindings = [
            ...
            (f"plate-{row['plateId']}-130-505", 130.0, 505),
            (f"{ENTRY_PREFIX}-{row['plateId']}-505-600", 505, 600),
        ]
        ...
        for quarter in range(410 * 4, 600 * 4 + 1):
            age = quarter / 4
            ...
            binding = sorted(matches, key=lambda item: item["entryId"])[0]
            ...
            if error > oracle_max:
                oracle_max, oracle_age = error, age
    if oracle_max > contract["motionExtensionMa"]["angularErrorLimitRad"]:
        raise BuildError(f"Barents shelf motion residual {oracle_max} exceeds contract")
    return {
...
def apply(package: Path) -> dict:
    ...
    cp, rotation, source_clock, rows_by_plate = build_motion_rows(contract)
    if any(entry["entryId"].startswith(f"{ENTRY_PREFIX}-") for entry in palette["entries"]):
        raise BuildError("Barents shelf motion entries already exist")
    if any(interval["id"] == INTERVAL_ID for interval in palette["sourceIntervalSets"]):
        raise BuildError("Barents shelf motion interval already exists")
    ...
    for plate, new_rows in rows_by_plate.items():
        ...
        entry = {
            "entryId": f"{ENTRY_PREFIX}-{plate}-505-600",
            "plateId": plate,
    ...
    palette["sourceIntervalSets"].append({
        "id": INTERVAL_ID,
        "intervals": [
            {"youngestAgeMa": 505, "oldestAgeMa": 600, "kind": "smooth-motion"},
    ...
    for row in contract["charts"]:
        chart = chart_by_id[row["chartId"]]
        if chart["lifecycle"]["validTimeMa"]["oldest"] != 410:
            raise BuildError(f"{row['chartId']}: baseline lifecycle is not 410 Ma")
        ...
        if older is None or older["validTimeMa"] != {"youngest": 130.0, "oldest": 410}:
        ...
        bindings.append({
            ...
            "entryId": f"{ENTRY_PREFIX}-{row['plateId']}-505-600",
            "validTimeMa": {"youngest": 505, "oldest": 600},
        })
    ...
    if changed != expected or core["spatialBatches"] != before_geometry:
        raise BuildError("repair changed geometry or charts outside the three exact targets")
...
def main() -> None:
    ...
    result = (apply(args.package) if args.apply else validate_applied(args.package)
              if args.validate_applied else check(args.package))
