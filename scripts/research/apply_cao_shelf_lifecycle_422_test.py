#!/usr/bin/env python3
"""Regression and mutation tests for the six-chart Cao 422 Ma shelf repair."""
...
MODULE = Path(__file__).with_name("apply_cao_shelf_lifecycle_422.py")
SPEC = importlib.util.spec_from_file_location("apply_cao_shelf_lifecycle_422", MODULE)
assert SPEC and SPEC.loader
repair = importlib.util.module_from_spec(SPEC)
SPEC.loader.exec_module(repair)


class CaoShelfLifecycle422Test(unittest.TestCase):
    def setUp(self) -> None:
        self.temp = tempfile.TemporaryDirectory(prefix="earthhistory-cao-shelf-422-")
        ...
        if any(entry["entryId"].startswith(f"{repair.ENTRY_PREFIX}-")
            ...
            self.restore_pre_repair_candidate()

    def tearDown(self) -> None:
        self.temp.cleanup()

    def restore_pre_repair_candidate(self) -> None:
        ...
        palette["sourceIntervalSets"] = [row for row in palette["sourceIntervalSets"]
                                         if not row["id"].startswith(repair.INTERVAL_PREFIX)]
        ...
        for row in contract["charts"]:
            ...
            chart["lifecycle"]["validTimeMa"]["oldest"] = row["compiledOldestMa"]
            chart["motionBindings"] = [binding for binding in chart["motionBindings"]
                                       if binding["entryId"] != repair.extension_entry_id(row)]
            next(binding for binding in chart["motionBindings"]
                 if binding["entryId"] == f"plate-{row['plateId']}-130-505")["validTimeMa"]["oldest"] = row["compiledOldestMa"]
    ...
    def test_baseline_reproduces_all_six_422_ma_omissions(self) -> None:
        self.assertEqual(repair.check(self.package)["baselineMissingAt422"], [True] * 6)
    ...
    def test_exact_identity_repair_preserves_geometry_and_coast_charts(self) -> None:
        ...
        self.assertEqual(result["changedChartIndices"], [100, 207, 232, 391, 763, 858])
        ...
        changed = [index for index, pair in enumerate(zip(before["charts"], after["charts"]))
                   if pair[0] != pair[1]]
        self.assertEqual(changed, [100, 207, 232, 391, 763, 858])
    ...
    def test_repaired_lifecycle_includes_600_and_excludes_older_epsilon(self) -> None:
        ...
        self.assertEqual(result["lifecycleWitnesses"], {
            ...
            "420": [True] * 6, "420.000001": [True] * 6,
            "422": [True] * 6, "600": [True] * 6,
            "600.000001": [False] * 6,
        })
    ...
    def test_chart_identity_mutation_is_rejected(self) -> None:
        core_path = self.package / "core.json"
        core = package_intern.read_package_json(core_path)
        target_index = json.loads(repair.CONTRACT.read_text())["charts"][0]["compiledChartIndex"]
        core["charts"][target_index]["sourceFeatureIds"] = ["GPlates-wrong"]
    ...
    def test_truncated_lifecycle_mutation_reproduces_the_422_failure(self) -> None:
        ...
        target["lifecycle"]["validTimeMa"]["oldest"] = 420
        core_path = self.package / "core.json"
        core_path.write_bytes(repair.canonical(package_intern.intern_charts(core)))
        self.rebind_core_identity(core_path)
